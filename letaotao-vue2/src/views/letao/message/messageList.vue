<template>
  <el-main>
    ...
    >
      ...
      <el-table-column prop="receiverName" label="接收者"></el-table-column>
      ...
      <el-table-column align="center" width="100" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="hasPermission('ltt:message:delete')"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      // 查询条件对象
      messageSearchModel: { content: "", type: "", nickName: "" },
      messageList: [], // 通知/消息列表
      tableHeight: 0, // 表格高度
      pageNo: 1, // 当前页码
      pageSize: 10, // 每页显示数量
  ...
  methods: {
    ...
    async search(pageNo = 1, pageSize = 10) {
      let params = {
        pageNo: pageNo,
        pageSize: pageSize,
        ...this.messageSearchModel,
      };
      // 发送查询请求
      let res = await messageApi.getMessageList(params);
      if (res.success) {
    ...
    resetValue() {
      // 清空查询条件
      this.$resetForm("searchForm", this.messageSearchModel);
      // 重新查询
      this.search();
    },
    /** * 删除 */
    async handleDelete(row) {
      ...
      if (confirm) {
        ...
        // 发送删除请求
        let res = await messageApi.deleteMessageById(params);
        // 判断是否成功
        if (res.success) {
